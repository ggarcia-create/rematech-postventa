...
export function documentHTML(d, kind, images) {
  if (kind === "ticket")
    return `<article class="paper ticket ${Number(d.ticketWidth) === 58 ? "narrow" : ""}"><header>${brand}<h3>RECEPCIÓN DE EQUIPO</h3><p>Comprobante de servicio</p></header>${metadata(d)}<h4>DATOS DEL CLIENTE</h4>${row("Cliente", d.client)}${row("Correo", d.clientEmail)}${row("Pedido", d.order)}${row("Tipo de ingreso", d.intakeType || "No especificado")}${row("Equipo", d.equipment)}${row("Serie", d.serial)}<h4>FALLA REPORTADA</h4>${row("Motivo", d.reason)}<p class="reported">${e(d.description || "Sin descripción capturada.")}</p><h4>DECLARACIÓN DE GARANTÍA</h4><p class="warranty">${e(warranty(d.client))}</p><footer><strong>REMATECH MÉXICO</strong>Conserva este comprobante para cualquier seguimiento.</footer></article>`;
