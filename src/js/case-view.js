...
export function caseDetailHTML(r, user, images) {
  ...
  const returnSection =
    r.intake?.intakeType === "Devolución"
      ? `<section class="case-block return-status"><h3>SEGUIMIENTO DE DEVOLUCIÓN</h3><p class="help">Actualiza el resultado y el destino físico del equipo. Regresó a Full archiva el expediente; Retiro creado lo mueve a Retiros.</p><form id="return-form"><div class="technical-form"><label>Estado actual<select name="resolution" ${returnEditable ? "" : "disabled"}>${RETURN_RESOLUTIONS.map((value) => `<option ${r.returnResolution === value ? "selected" : ""}>${e(value)}</option>`).join("")}</select></label><label>Destino del equipo<select name="destination" ${returnEditable ? "" : "disabled"}>${RETURN_DESTINATIONS.map((value) => `<option ${r.destination === value ? "selected" : ""}>${e(value)}</option>`).join("")}</select></label>${field("Número de retiro", "withdrawalNumber", r.withdrawalNumber || "", { disabled: !returnEditable })}</div>${returnEditable ? '<div class="case-actionbar"><button type="button" data-case-action="update-return" class="primary">Guardar devolución</button></div>' : ""}</form></section>`
      : "";
