...
Deno.serve(async request=>{
 ...
 if(request.method!=='POST')return answer({error:'Método no permitido.'},405);
 try {
  if(Number(request.headers.get('content-length')||0)>7000000)return answer({error:'Solicitud demasiado grande.'},413);
  const db=createClient(Deno.env.get('SUPABASE_URL')!,Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,{auth:{persistSession:false,autoRefreshToken:false}});
  const token=request.headers.get('Authorization')?.replace(/^Bearer /,'');
  if(!token)return answer({error:'Inicia sesión.'},401);
  const {data:{user:identity},error}=await db.auth.getUser(token);
  if(error||!identity)return answer({error:'Tu sesión expiró. Vuelve a entrar.'},401);
  const profile=check(await db.from('rematech_profiles').select('*').eq('id',identity.id).single());
  ...
  if(input.action==='change-password'){
   ...
   const attempt=await probe.auth.signInWithPassword({email:profile.email,password});
   if(!attempt.error){await probe.auth.signOut({scope:'local'});throw new Error('Usa una contraseña distinta a la actual.');}
   check(await db.auth.admin.updateUserById(identity.id,{password}));
   check(await db.from('rematech_profiles').update({must_change_password:false}).eq('id',identity.id));
  ...
  if(input.action==='reset-user-password'){
   admin();if(input.id===user.id)throw new Error('No puedes restablecer tu propia contraseña aquí.');
   if(typeof input.password!=='string'||input.password.length<10)throw new Error('La contraseña temporal debe tener al menos 10 caracteres.');
   const target=check(await db.from('rematech_profiles').select('id').eq('id',input.id).single());
   check(await db.from('rematech_profiles').update({must_change_password:true}).eq('id',target.id));
   check(await db.auth.admin.updateUserById(target.id,{password:input.password}));
  ...
  if(input.action==='register'||input.action==='import-local'){
   ...
   try{
    for(let i=0;i<files.length;i++){
     ...
     const {type,base64}=files[i];if(!['image/jpeg','image/png','image/webp'].includes(type)||typeof base64!=='string'||base64.length>2800000)throw new Error('Evidencia no válida.');
  ...
  if(input.action==='get'){
   ...
   record.evidence=await Promise.all(record.evidence.map(async(path:string|null)=>path?check(await db.storage.from('rematech-evidence').createSignedUrl(path,120)).signedUrl:null));return answer(record);
  }
  if(input.action==='read'){
   const note=(row.body.notifications||[]).find((n:any)=>n.id===input.notificationId&&n.targets.includes(user.role));
   if(!note)throw new Error('Notificación no disponible.');
   check(await db.from('rematech_notification_reads').upsert({user_id:user.id,case_id:input.id,notification_id:note.id}));return answer({ok:true});
  }
  if(input.action==='act'){
   if(row.revision!==input.revision)throw new Error('Otro usuario actualizó el expediente. Vuelve a abrirlo.');
   const next=applyAction(row.body,input.operation,input.payload||{},user);
   check(await db.rpc('rematech_save_case',{p_id:input.id,p_revision:input.revision,p_body:next}));return answer({...next,evidence:[]});
  }
  return answer({error:'Acción no válida.'},400);
 }catch(error){return answer({error:error instanceof Error?error.message:'No se pudo completar la operación.'},400);}
});
